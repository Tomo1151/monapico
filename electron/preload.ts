...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  showExplorerContextMenu: (path: string, isDirectory: boolean, canDelete: boolean = true) => ipcRenderer.send('explorer:showContextMenu', path, isDirectory, canDelete),
  
  onCreateNewFile: (callback: (data: { path: string, isDirectory: boolean }) => void) => {
  ...
  onCreateNewFolder: (callback: (data: { path: string, isDirectory: boolean }) => void) => {
    const listener = (_event: any, data: any) => callback(data);
  ...
  onDeleteItem: (callback: (data: { path: string, isDirectory: boolean }) => void) => {
    const listener = (_event: any, data: any) => callback(data);
