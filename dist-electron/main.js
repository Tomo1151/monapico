...
const RAW_REPL_ENTER = "";
const RAW_REPL_EXIT = "";
const RAW_REPL_INTERRUPT = "";
const RAW_REPL_EOT = "";
const RAW_REPL_READY_TOKEN = "raw REPL; CTRL-B to exit";
const RAW_REPL_DEFAULT_TIMEOUT_MS = 4e3;
let serialCapture = null;
...
const isPicoPath = (value) => value.startsWith(PICO_PATH_PREFIX);
const toDevicePath = (picoPath) => {
  const stripped = picoPath.replace(/^pico:\/*/i, "");
  const normalized = `/${stripped}`.replace(/\/+$/g, "");
  return normalized === "" ? "/" : normalized.replace(/\/+/g, "/");
};
const toPicoPath = (devicePath) => {
...
const detectPicoConnection = async (ports) => {
...
const rejectSerialCapture = (error) => {
  if (!serialCapture) return;
  clearTimeout(serialCapture.timer);
...
const awaitSerialOutput = (match, timeoutMs) => {
  const port = picoSerialPort;
  if (!(port == null ? void 0 : port.isOpen)) {
...
const ensurePicoSerialConnected = async () => {
  if (picoSerialPort == null ? void 0 : picoSerialPort.isOpen) return true;
  const ports = await listSerialPorts();
...
const picoFsList = async (picoPath) => {
...
const picoReadFile = async (picoPath) => {
  const connected = await ensurePicoSerialConnected();
...
const picoMkdir = async (picoPath) => {
  const connected = await ensurePicoSerialConnected();
  if (!connected) return false;
  const devicePath = toDevicePath(picoPath);
  const safePath = escapePyString(devicePath);
